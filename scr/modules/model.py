...
class Transform:
    ...
    def __init__(self, bs, **kwargs):
        ...
        if TEST_MODE:
            print('|TEST WARNING| Transform.noise = np.ones')
            noise = np.ones((bs, 2, 3)).astype(np.float32)
        self.theta = dygraph.to_variable(noise + np.eye(2, 3).reshape(1, 2, 3))
        self.bs = bs
        ...
        if ('sigma_tps' in kwargs) and ('points_tps' in kwargs):
            ...
            if TEST_MODE:
                print('|TEST WARNING| Transform.control_params = np.ones')
                self.control_params = dygraph.to_variable(np.ones(self.control_params.shape))
    ...
    def transform_frame(self, frame):
        ...
        if TEST_MODE:
            ...
            print('|TEST WARNING|Output of fluid.layers.grid_sampler is 2. model:L152')
            return fluid.dygraph.to_variable(np.ones(bf.shape).astype(np.float32)*2)
        # 0.0.0c 分支等待更新
        elif paddle.version.full_version == '0.0.0c':
    ...
    #     grad_x = dygraph.grad(fluid.layers.reduce_sum(new_coordinates[:, :, 0]), coordinates, create_graph=False)
    #     grad_y = dygraph.grad(fluid.layers.reduce_sum(new_coordinates[:, :, 1]), coordinates, create_graph=False)
    ...
    def jacobian(self, coordinates):
        new_coordinates, (grad_x, grad_y) = self.warp_coordinates(coordinates, True)  # When batch_size is 5, the shape of coordinates and new_coordinates is (5, 10, 2)
        assert len(new_coordinates.shape) == 3
        jacobian = fluid.layers.concat([fluid.layers.unsqueeze(grad_x, [-2]), fluid.layers.unsqueeze(grad_y, [-2])],
                                       axis=-2)
        return jacobian
    ...
    def forward(self, x):
        ...
        if (self.loss_weights['equivariance_value'] + self.loss_weights['equivariance_jacobian']) != 0:
            ...
            if self.loss_weights['equivariance_jacobian'] != 0:
                jacobian_transformed = fluid.layers.matmul(*broadcast_v1(transform.jacobian(transformed_kp['value']), transformed_kp['jacobian']))
...
def broadcast_v2(x, y):
    ...
    if np.count_nonzero(max_shape%np.array(dim_x)) != 0 or np.count_nonzero(max_shape%np.array(dim_y)) != 0:
        raise ValueError()
    x_bc = fluid.layers.expand(x, ((max_shape/np.array(dim_x)).astype(np.int32).tolist())).astype('float32')
    y_bc = fluid.layers.expand(y, ((max_shape/np.array(dim_y)).astype(np.int32).tolist())).astype('float32')
    return x_bc, y_bc
def _inverse(x):
    """手动求逆以兼容1.8.x集群
    """
